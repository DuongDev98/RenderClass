...
NAME = "";
                        ...
                        public DNHOMHANGRow(DataRow row)
                        {
                            InitWithDataRow(row);
                        }
...
if (row.Table.Columns.Contains("NAME") && row["NAME"] != DBNull.Value) NAME = row["NAME"].ToString();
                        ...
                        public DNHOMHANGRow(string ID)
                {
                    this.ID = ID;
                    if (ID == null || ID.Length == 0) InitWithoutParam();
                    else
                    {
                        DataRow row = db.GetFirstRow("select * from DNHOMHANG where id = @id", attrs);
                        if (row != null)
                        {
                            InitWithDataRow(row);
                        }
...
row["NAME"] = NAME;
                ...
                public bool Update()
                {
                    ...
                    if (insert)
                    {
                        querySql = "insert into DNHOMHANG(" + p1 + ") values(" + p2 + ")";
                    }
                    else
                    {
                        querySql = "update DNHOMHANG set " + querySql + @" where ID = @ID";
                    }
                ...
                public void Delete()
                {
                    db.ExSql("delete DNHOMHANG from id = @id", attrs);
                }
                    }
                }