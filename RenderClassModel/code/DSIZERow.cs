...
if (row.Table.Columns.Contains("NAME") && row["NAME"] != DBNull.Value) NAME = row["NAME"].ToString();
                        ...
                        public DSIZERow(string ID)
                {
                    this.ID = ID;
                    if (ID == null || ID.Length == 0) InitWithoutParam();
                    else
                    {
                        DataRow row = db.GetFirstRow("select * from DSIZE where id = @id", attrs);
                        if (row != null)
                        {
                            InitWithDataRow(row);
                        }
...
row["NAME"] = NAME;
                ...
                public bool Update()
                {
                    ...
                    if (insert)
                    {
                        querySql = "insert into DSIZE(" + p1 + ") values(" + p2 + ")";
                    }
                    else
                    {
                        querySql = "update DSIZE set " + querySql + @" where ID = @ID";
                    }
                ...
                public void Delete()
                {
                    db.ExSql("delete DSIZE from id = @id", attrs);
                }
                    }
                }